...
import { getSeniorCitizenBySlug, getAllSeniorCitizenSlugs } from "@/lib/content";
import {
...
export async function generateStaticParams() {
  return getAllSeniorCitizenSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const item = getSeniorCitizenBySlug(slug);
  if (!item) return {};
  const url = `https://knowledgekendra.com/senior-citizen/${slug}`;
  return {
    title: `${item.title} - Senior Citizen Schemes`,
    ...
    openGraph: {
      ...
      type: "article",
      images: [{
        url: `/api/og?title=${encodeURIComponent(item.title)}&icon=${encodeURIComponent(item.hero?.icon || '👴')}&cat=senior-citizen`,
...
export default async function SeniorCitizenDetailPage({ params }: Props) {
  const { slug } = await params;
  const g = getSeniorCitizenBySlug(slug) as any;
  if (!g) notFound();
  // Build TOC items
  ...
  return (
    <div className="max-w-[860px] mx-auto px-5 py-6">
      ...
      <JsonLd data={breadcrumbSchema([
        ...
        { name: "Senior Citizen Schemes", url: "https://knowledgekendra.com/senior-citizen" },
        { name: g.title, url: `https://knowledgekendra.com/senior-citizen/${slug}` },
      ])} />
      {g.faqs?.length > 0 && <JsonLd data={faqSchema(g.faqs)} />}
      <JsonLd data={articleSchema({
        title: g.title,
        description: g.meta_description || g.hero.one_liner,
        url: `https://knowledgekendra.com/senior-citizen/${slug}`,
      ...
      <Breadcrumb
        items={[
          { label: "Home", href: "/" },
          { label: "Senior Citizen Schemes", href: "/sarkari-naukri" },
          { label: g.title },
        ]}
      />

      <div className="flex justify-end mb-2"><SaveForLater slug={slug} title={g.title} url={`/senior-citizen/${slug}`} /></div>
      <HeroBanner
        title={g.title}
      ...
      {g.sections.map((section: any, idx: number) => {
        const sectionId = section.heading && section.type !== "svg_block"
          ? `section-${g.sections.filter((s: any, i: number) => i < idx && s.heading && s.type !== "svg_block").length}`
          : undefined;
        return (
      ...
      {g.related_pages?.length > 0 && (
        <div className="mt-10 flex flex-wrap gap-2">
          {g.related_pages.map((r: any) => (
            <a key={r.slug} href={`/senior-citizen/${r.slug}`} className="px-4 py-2 rounded-lg text-sm bg-card border border-border hover:border-accent/40 text-text-secondary transition-colors">{r.title} →</a>
          ))}
        </div>
      )}
