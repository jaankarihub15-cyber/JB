...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      images: [{
        url: `/api/og?title=${encodeURIComponent(exam.title)}&icon=${encodeURIComponent(exam.hero?.icon || '📝')}&cat=exam`,
        width: 1200, height: 630, alt: exam.title,
      }],
    },
...
export default async function ExamDetailPage({ params }: Props) {
  ...
  // Sticky nav anchors: only show ones the page can actually scroll to.
  ...
  // TOC (right rail) from extra_sections
  const tocItems = (e.extra_sections || [])
    .filter((s: any) => s.heading && s.type !== "svg_block")
    .map((s: any, i: number) => ({ id: `section-${i}`, text: s.heading.replace(/<[^>]+>/g, '').substring(0, 50) }));
  if (e.faqs?.length > 0) tocItems.push({ id: "faqs", text: "FAQs" });
  ...
  return (
    <div className="theme-v2 py-0">
      ...
      <div className="hero-band-v2">
        <div className="max-w-[1140px] mx-auto px-5 md:px-6">
          ...
          <div className="pt-3 flex items-center gap-2.5 flex-wrap">
            <span className="text-[11px] font-semibold tracking-wide text-[#8FB8A2] uppercase">
              {e.category || "Government Exam"}
            </span>
      ...
      <div className="max-w-[1140px] mx-auto px-5 md:px-6">
        ...
        {/* ANSWER-FIRST LAYER (all optional, hides when absent) */}
        <AnswerRouter items={e.answer_router} />
        <FactTiles facts={e.quick_facts} />

        {/* SALARY LEADS THE BODY */}
        ...
        {/* Exam pattern */}
        {e.exam_pattern && e.exam_pattern.length > 0 && e.exam_pattern.map((tier: any) => (
        ...
        {/* Posts & Salary (full detail retained below the lead summary) */}
        ...
        {e.extra_sections && e.extra_sections.map((section: any, idx: number) => {
          ...
          <div id={sectionId}>
            ...
            {section.type === "info_rows" && (
              <Card>{section.rows.map((row: any) => (<InfoRow key={row.label} label={row.label} value={row.value} highlight={row.highlight} />))}</Card>
            )}
            ...
            <Card className="px-6 py-5">
              {e.preparation_tips.map((tip: string, i: number) => (
                <div key={i} className={`py-3 text-base text-text-secondary leading-relaxed flex gap-3 ${i < e.preparation_tips.length - 1 ? "border-b border-border" : ""}`}>
                  <span className="text-accent font-bold shrink-0">{i + 1}.</span><span dangerouslySetInnerHTML={{ __html: tip }} />
                </div>
              ))}
            </Card>
          </>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {e.related_pages.map((r: any) => (
                <a key={r.slug} href={r.url || `/exam/${r.slug}`} className="card px-5 py-4 min-w-[200px] shrink-0 hover:border-accent/30">
                  <Tag className="bg-blue-light">{r.tag}</Tag><div className="text-base font-medium text-text mt-2">{r.title}</div>
                </a>
              ))}
            </div>
          </>
