...
// CHANGED: removed TableOfContents, added HeroV2 + answer-layer
import { HeroV2 } from "@/components/hero-v2";
import { StatusPill, AnswerRouter, FactTiles, SalaryLead } from "@/components/exam-answer-layer";
import { notFound } from "next/navigation";
import { getExamBySlug, getAllExamSlugs } from "@/lib/content";
import {
  Breadcrumb, SectionHeading, Card, InfoRow, StepCard, FAQ, Tag,
} from "@/components/ui";
// CHANGED: removed HeroBanner (now using HeroV2 + inline hero band)
import { ExamTabs } from "@/components/exam-tabs";
import { JsonLd, faqSchema, breadcrumbSchema } from "@/components/json-ld";
// CHANGED: removed articleSchema (unused)
...
export default async function ExamDetailPage({ params }: Props) {
  ...
  return (
    <div className="theme-v2 py-0">
      ...
      {/* CHANGED: V2 hero band replaces HeroBanner */}
      <div className="hero-band-v2">
        <div className="max-w-[860px] mx-auto px-6">
          ...
          <div className="mt-3">
            <StatusPill status={e.status || e.hero?.status} />
          </div>
          <HeroV2
            title={e.title}
      ...
      {/* CONTENT COLUMN — same width as before, flat layout */}
      <div className="max-w-[860px] mx-auto px-6">

        {/* CHANGED: simple sticky nav (one nav, no TOC) */}
        {navItems.length >= 2 && (
          <nav className="sticky top-0 z-20 -mx-6 px-6 bg-white/95 backdrop-blur border-b border-border">
            <div className="flex gap-1 overflow-x-auto whitespace-nowrap py-2.5">
              {navItems.map((it, i) => (
                <a key={it.id} href={`#${it.id}`} className={`text-[12.5px] font-semibold px-3 py-1.5 rounded-full shrink-0 no-underline transition-colors ${i === 0 ? "bg-[#E1F5EE] text-[#1B6B4A]" : "text-[#5f5e5a] hover:bg-[#F4F3EF]"}`}>{it.label}</a>
              ))}
            </div>
          </nav>
        )}

        <div className="flex justify-end mb-2 mt-3"><SaveForLater slug={slug} title={e.title} url={`/exam/${slug}`} /></div>

        {/* CHANGED: answer-first blocks (all optional, hide when absent) */}
        <AnswerRouter items={e.answer_router} />
        <FactTiles facts={e.quick_facts} />

        {/* CHANGED: salary FIRST (was after exam_pattern) */}
        <SalaryLead salary={e.salary_table} posts={e.major_posts} />

        {/* Exam Prep Tabs — Syllabus, Books, Study Plan, Quiz (unchanged) */}
        <ExamTabs data={{
        ...
        {/* Key Details (unchanged, added id) */}
        ...
        {/* Exam Pattern (unchanged, added id on first tier) */}
        {e.exam_pattern && e.exam_pattern.length > 0 && e.exam_pattern.map((tier: any, ti: number) => (
          <div key={tier.tier_name} id={ti === 0 ? "syllabus" : undefined}>
            ...
            <Card className="px-6 py-5">
              {tier.description && (
                <p className="text-base text-text-secondary leading-[1.75] mb-5" dangerouslySetInnerHTML={{ __html: tier.description }} />
              )}
        ...
        {/* Posts & Salary full detail — show only if salary_table exists (otherwise SalaryLead already showed this data) */}
        ...
        {/* Extra Sections — deep content (UNCHANGED from working renderer) */}
        {e.extra_sections && e.extra_sections.map((section: any, idx: number) => {
          ...
          <div id={sectionId}>
            ...
            {section.type === "steps" && (
              <Card>{section.steps.map((step: any) => (<StepCard key={step.step} number={step.step} title={step.title} description={step.description} />))}</Card>
            )}

            {section.type === "table" && (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse card overflow-hidden text-sm">
                  <thead><tr className="bg-card-alt">{section.columns.map((col: string) => (<th key={col} className="px-4 py-3 text-left font-semibold text-text border-b border-border">{col}</th>))}</tr></thead>
                  <tbody>{section.rows.map((row: string[], ri: number) => (<tr key={ri}>{row.map((cell: string, ci: number) => (<td key={ci} className={`px-4 py-3 border-b border-border ${ci === 0 ? "font-medium text-text" : "text-text-secondary"}`} dangerouslySetInnerHTML={{ __html: cell }} />))}</tr>))}</tbody>
                </table>
              </div>
            )}
            ...
            {section.type === "stat_cards" && (
              <div className="my-5 grid grid-cols-2 md:grid-cols-4 gap-3">
                {section.cards.map((card: any, i: number) => (
                  <div key={i} className={`card p-4 text-center ${card.variant === "accent" ? "bg-accent-light border-accent" : ""}`}>
            ...
            {section.type === "divider" && (
              <div className="my-8 flex items-center gap-3"><div className="flex-1 h-px bg-border"></div>{section.label && <span className="text-xs text-text-muted uppercase tracking-wider font-semibold">{section.label}</span>}<div className="flex-1 h-px bg-border"></div></div>
            )}
          </div>
          {idx === 0 && <ResizerCTA />}
          </Fragment>
        ...
        {/* Important Dates (unchanged, added id) */}
        ...
        <div className="mt-8 border-t border-border pt-6">
          <PdfSummary title={e.title} stats={e.hero?.stats} faqs={e.faqs} />
          <SourceCitations />
        </div>
