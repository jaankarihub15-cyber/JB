...
import { getAllWomen } from "@/lib/content";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Women & Girl Schemes - KnowledgeKendra",
  description: "Guides on schemes for women and girls. Eligibility, amounts, application steps and status checks explained simply.",
  alternates: { canonical: "https://knowledgekendra.com/women" },
};

export default function WomenHubPage() {
  const items = getAllWomen();
    ...
    <div className="max-w-[1100px] mx-auto px-6">
      <div className="mt-8 mb-8 bg-card border border-border rounded-3xl p-6 md:p-8">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-text">Women & Girl Schemes</h1>
            <p className="text-sm text-text-secondary">{items.length} articles - schemes for women and girls</p>
          </div>
        </div>
        <div className="flex gap-2 mt-4 flex-wrap">
          {["Maternity", "Girl Child", "Pension", "Self-Help", "Safety", "Scholarship"].map((t) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-12">
        {items.map((e: any) => (
          <Link key={e.slug} href={`/women/${e.slug}`} className="bg-card border border-border rounded-2xl p-4 hover:border-green/40 hover:shadow-sm hover:-translate-y-px transition-all">
