...
import { JsonLd, breadcrumbSchema, faqSchema, specialAnnouncementSchema } from "@/components/json-ld";
import fs from "fs";
  ...
  return (
    <div className="max-w-[860px] mx-auto px-5 py-6">
      ...
      <JsonLd data={specialAnnouncementSchema({
        ...
        url: `https://knowledgekendra.com/news/${slug}`,
      ...
      {article.official_links?.length > 0 && (
        <div className="my-6 p-5 bg-card border border-border rounded-2xl">
          <p className="text-sm font-bold text-text mb-3 flex items-center gap-2">🔗 Official Links</p>
          <div className="flex flex-col gap-2">
            {article.official_links.map((link: any) => (
              <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer" className="text-sm text-accent hover:underline flex items-center gap-1.5">
