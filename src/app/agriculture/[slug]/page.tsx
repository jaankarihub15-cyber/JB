...
import { getAgricultureBySlug, getAllAgricultureSlugs } from "@/lib/content";
import {
...
export async function generateStaticParams() {
  return getAllAgricultureSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const item = getAgricultureBySlug(slug);
  if (!item) return {};
  const url = `https://knowledgekendra.com/agriculture/${slug}`;
  return {
    title: `${item.title} - Agriculture & Farmer Schemes`,
    ...
    openGraph: {
      ...
      type: "article",
      images: [{
        url: `/api/og?title=${encodeURIComponent(item.title)}&icon=${encodeURIComponent(item.hero?.icon || '🌾')}&cat=agriculture`,
...
export default async function AgricultureDetailPage({ params }: Props) {
  const { slug } = await params;
  const g = getAgricultureBySlug(slug) as any;
  if (!g) notFound();
  // Build TOC items
  ...
  return (
    <div className="max-w-[860px] mx-auto px-5 py-6">
      ...
      <JsonLd data={breadcrumbSchema([
        ...
        { name: "Agriculture & Farmer Schemes", url: "https://knowledgekendra.com/agriculture" },
        { name: g.title, url: `https://knowledgekendra.com/agriculture/${slug}` },
      ])} />
      {g.faqs?.length > 0 && <JsonLd data={faqSchema(g.faqs)} />}
      <JsonLd data={articleSchema({
        title: g.title,
        description: g.meta_description || g.hero.one_liner,
        url: `https://knowledgekendra.com/agriculture/${slug}`,
      ...
      <Breadcrumb
        items={[
          { label: "Home", href: "/" },
          { label: "Agriculture & Farmer Schemes", href: "/sarkari-naukri" },
          { label: g.title },
        ]}
      />

      <div className="flex justify-end mb-2"><SaveForLater slug={slug} title={g.title} url={`/agriculture/${slug}`} /></div>
      <HeroBanner
        title={g.title}
      ...
      {g.related_pages?.length > 0 && (
        <div className="mt-10 flex flex-wrap gap-2">
          {g.related_pages.map((r: any) => (
            <a key={r.slug} href={`/agriculture/${r.slug}`} className="px-4 py-2 rounded-lg text-sm bg-card border border-border hover:border-accent/40 text-text-secondary transition-colors">{r.title} →</a>
          ))}
        </div>
      )}
