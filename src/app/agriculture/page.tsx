...
import { getAllAgriculture } from "@/lib/content";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Agriculture & Farmer Schemes - KnowledgeKendra",
  description: "Guides on schemes and support for farmers. Eligibility, amounts, application steps and status checks explained simply.",
  alternates: { canonical: "https://knowledgekendra.com/agriculture" },
};

export default function AgricultureHubPage() {
  const items = getAllAgriculture();
    ...
    <div className="max-w-[1100px] mx-auto px-6">
      <div className="mt-8 mb-8 bg-card border border-border rounded-3xl p-6 md:p-8">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-text">Agriculture & Farmer Schemes</h1>
            <p className="text-sm text-text-secondary">{items.length} articles - schemes and support for farmers</p>
          </div>
        </div>
        <div className="flex gap-2 mt-4 flex-wrap">
          {["Crop Insurance", "Subsidy", "Credit", "Irrigation", "PM-Kisan", "Mandi"].map((t) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-12">
        {items.map((e: any) => (
          <Link key={e.slug} href={`/agriculture/${e.slug}`} className="bg-card border border-border rounded-2xl p-4 hover:border-green/40 hover:shadow-sm hover:-translate-y-px transition-all">
