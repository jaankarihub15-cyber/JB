  ...
  return (
    <CalcCard>
      <div className="space-y-3 mb-5">
        {subjects.map((sub, i) => (
          <div key={i} className="flex flex-col sm:flex-row gap-2 sm:items-center bg-card-alt sm:bg-transparent rounded-lg p-2 sm:p-0">
            <input
              type="text"
              value={sub.name}
              onChange={(e) => updateSubject(i, "name", e.target.value)}
              className="w-full sm:flex-1 px-3 py-2 rounded-lg bg-card sm:bg-card-alt text-sm text-text border-none outline-none focus:ring-2 focus:ring-accent/30"
            />
            <div className="flex gap-2 items-center">
              <input
                ...
                className="w-14 sm:w-16 px-2 py-2 rounded-lg bg-card sm:bg-card-alt text-sm text-center text-text border-none outline-none focus:ring-2 focus:ring-accent/30"
                min={1} max={10}
                aria-label="Credits"
              />
              <select
                ...
                className="flex-1 sm:flex-none px-2 py-2 rounded-lg bg-card sm:bg-card-alt text-sm text-text border-none outline-none focus:ring-2 focus:ring-accent/30"
                aria-label="Grade"
              >
                {Object.keys(gradePoints).map((g) => <option key={g} value={g}>{g}</option>)}
              </select>
              <button onClick={() => removeSubject(i)} className="text-text-muted hover:text-red-500 text-lg px-1 shrink-0" aria-label="Remove subject">×</button>
            </div>
          </div>
        ))}
      </div>
