...
export function InfoRow({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start py-3.5 border-b border-border gap-1 sm:gap-4">
      <span className="text-sm text-text-muted shrink-0">{label}</span>
      <span className={`text-sm text-left sm:text-right ${highlight ? "font-bold text-accent" : "text-text font-medium"}`} dangerouslySetInnerHTML={{ __html: value }} />
    </div>
  );
}
